...
class JobController extends Controller
{
    ...
    public function editarSubmit(Request $request, Vaga $vaga)
    {
        $this->authorize('update', $vaga); 
        ...
        'cargo'     => 'required|max:255',
        'link_vaga' => 'required|url|max:2048',
        'status'    => 'required|in:aplicado,em_andamento,entrevista,aprovado,reprovado',
    ], [
        'empresa.required'   => 'O campo empresa é obrigatório.',
        'empresa.max'        => 'O nome da empresa deve ter no máximo 255 caracteres.',
        'cargo.required'     => 'O campo cargo é obrigatório.',
        ...
        $vaga->update($request->only(['empresa', 'cargo', 'link_vaga', 'anotacoes', 'status']));

        return redirect()->route('home');

    }

    public function deletar(Vaga $vaga)
    {
